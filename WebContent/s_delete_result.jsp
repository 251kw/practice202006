		...
		<div class="padding-y-5 text-center">
			...
			<form action="./top" method="post">
				<table class="table table-striped table-bordered table-hover">
					...
					<c:forEach var="shouts" items="${shouts}" varStatus="loop">
					<tbody>
						<tr>
							<td>${loop.count}</td>
							<td>${shouts.userName}</td>
							<td><span class="${shouts.icon} pe-2x pe-va"></span></td>
							<td><div class="balloon2-left">
								<p>${shouts.writing}</p></div></td>
						</tr>
					</c:forEach>
				</table>
					<input type="submit" value="掲示板に戻る" name="btn" class="btn">
				</form>
			</div>
		</div>
	</body>
